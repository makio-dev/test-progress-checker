...
flowchart TD
    ...
    subgraph rows["19行目以降 行チェック"]
        ...
        R2["C列が空 & D~V列に値あり → ケースIDなし"]
        R3["C列に値あり & Q列が空 → 実施日（予定）未入力"]
        R4["C列に値あり & S列が空 → 検証日（予定）未入力"]
...
| 9 | C列が空 & D\~V列に値あり | ケースIDがありません |
| 10 | C列に値あり & Q列が空 | ケースIDがあるのに実施日（予定）が未入力です |
| 11 | C列に値あり & S列が空 | ケースIDがあるのに検証日（予定）が未入力です |
| 12 | C列に値あり & O列がハイフン & V列が空 | 実施対象外ケースの場合は欠陥内容／備考欄に理由を記載してください |
| 13 | ITB- で始まるシートが1件のみであること | ITB- で始まるシートが N 件存在します |
